...
- Отказываемся от анемичных DTO и разделения на слои (MVC/MVVM) в пользу атомарных объектов Operation, инкапсулирующих поведение.
...
- Factory: Преобразование байтов в конкретный экземпляр Operation.
...
- Operation immutable, мутация через Intent → backend round-trip.
- Не публикует сырые данные.
...
- **Garbage Collection**:
  ...
  - Dead Stream Scanner: Фоновая задача для очистки "повисших" стримов (страховка).
...
- **Flow**: UI -> Operation -> Intent -> Backend -> Event -> New Operation -> UI.
- **Типизация**: Ответ на запрос типизируется ожидаемым классом Operation.
...
| 3. Fast Action (+/-) | Cmd -> Inf (Stream) | Optimistic / Laggy. UI не блокируется. Нет ожидания ответов. Цифры "догоняют" нажатия |
